...
namespace _2x2CRM.mvc.Models.UserIdentity
{
        ...
        IUserClaimStore<AppUser, Int64>
    {
        ...
        {
            await _dbContext.ExecuteAsync(null, "[CrmSecurity].[CreateUser]", user);
            CacheUser(user);
        }
        ...
        public async Task<AppUser> FindByIdAsync(Int64 userId)
        {
            ...
            if (user != null)
                return user;
            user = await _dbContext.LoadAsync<AppUser>(null, "[CrmSecurity].[FindUserById]", new { Id = userId });
            CacheUser(user);
            return user;
        }
        ...
        public async Task<AppUser> FindByNameAsync(String userName)
        {
            ...
            if (user != null)
                return user;
            user = await _dbContext.LoadAsync<AppUser>(null, "[CrmSecurity].[FindUserByName]", new { UserName = userName });
            CacheUser(user);
            return user;
        }
        ...
        public async Task UpdateAsync(AppUser user)
        {
            ...
            {
                await _dbContext.ExecuteAsync<AppUser>(null, "[CrmSecurity].[UpdateUserLockout]", user);
                user.ClearModified(UserModifiedFlag.Lockout);
            }
            else if (user.IsPasswordModified)
            {
                await _dbContext.ExecuteAsync<AppUser>(null, "[CrmSecurity].[UpdateUserPassword]", user);
        ...
        public async Task<AppUser> FindByEmailAsync(String email)
        {
            ...
            if (user != null)
                return user;
            user = await _dbContext.LoadAsync<AppUser>(null, "[CrmSecurity].[FindUserByEmail]", new { Email = email });
            CacheUser(user);
            return user;
        }
        ...
        {
            var list = await _dbContext.LoadListAsync<AppRole>(null, "[CrmSecurity].[GetUserGroups]", new { UserId = user.Id });
